...
def install_windows_wrapper(
    ...
    command = _encoded_install_command(_windows_path(source), spec.wrapper_path)
    try:
...
def _encoded_install_command(source: str, target: str) -> str:
    script = "\n".join(
        (
            f"$Source = {_powershell_literal(source)}",
