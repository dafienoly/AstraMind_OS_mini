...
- 已在最新主线基线重新形成单一 Alpha158 交付，未带入旧波次共享提交，也未修改
  WP-0070 公共合同；包内固定 `qlib-alpha158-computation-manifest-v3`，内容身份为
  `sha256:9c6871c861793952f6693e0c209b28a7179eb3df0c38d5e2a9ff40c6ddc5a5c6`。
- 158 个名称、公式及配对顺序分别保持固定哈希
  `d5f52c2d75ea900ab29f4742eeb59d9692254ba7307ad36a807012db7d680e13`、
  `05943b7d14e82ab604fe76938465589014c6e89b51f09116a2805c59080678a3` 和
  `b1154a5b310ec5f8ead6ca064c06ae4a1121dc1c9a0d4ff7dcd00efcf20704e5`。
- 普通 rolling 与固定 Qlib/Pandas 一致忽略 NaN、`+Inf`、`-Inf`；IdxMax/IdxMin
  继续按 Qlib 原始窗口保留独立 raw-NaN/非有限语义。另用全部源输入均为有限正数、
  但 WVMA5 中间比值溢出为 `+Inf` 的独立 oracle 锁定这一边界。
- Corr 严格按 Pandas 成对有效样本计算主体，再按 Qlib 运算次序使用原始左右序列的
  rolling std 执行 `atol=2e-5` 掩码。固定 1000 组、每组 16 行的随机差分 oracle
  覆盖 NaN、正负 Inf、近常量和大数；本地有限值及非有限分类与 Pandas 2.2.3
  逐单元一致，并冻结最小反例的 `+Inf/-Inf/-Inf/+Inf` 四种符号分类。
- 已把固定 Qlib 参考生成器源码作为可审计 fixture 提交，其 SHA256 为
  `7c4428980f86e3ac8b070a88b2be994fd28227946253587a1955276c47936950`；
  provenance 同时记录 Python compiler、实际 C compiler、编译后 `rolling` 扩展哈希、
  四个 Qlib 源文件哈希和隔离重生成命令。
- 独立 Qlib runtime 重生成的 `3 × 75 × 158` 结果及非有限掩码与已提交 golden
  逐字节一致。本地移植的 881 个非有限分类全部一致；最大绝对误差为 `0.1875`
  （`000001.SZ`、`2025-10-20`、`VSTD20`，参考值约 `4.365e14`），最大相对误差为
  `9.936390208552322e-11`（`300001.SZ`、`2025-11-27`、`STD5`），均通过逐项
  `rtol=1e-9, atol=1e-9`。
- 聚焦测试覆盖 full 原始三态映射、U0 单位、共同日历与内容身份、前缀不变、
  分块等价、证券重排/隔离、截止后输入和旧 computation manifest 攻击、公开输出
  非有限值转 `missing`；默认 runtime 不导入 Qlib、Pandas 或 SciPy。
