...
联合按 F0、Alpha158、Alpha101 和各包规范顺序拼接；跨包
`abs(median_daily_spearman) >= 0.85` 时复用 WP-0072 完全链接和代表规则去重，不修改
单包 manifest。至少一个两包联合在后续 WP-0074A 的开发期 C2 证据中相对较强单包为正，
三包联合才可标记 `combination_eligible`；本包可以训练和留存三包研究制品，但在该门
通过前不得发布为生产候选。
...
3. B0、十个 Ridge 配置预算和九 alpha 不可配置扩张。
