"""Publish company actions and snapshot-bound adjusted research prices."""
...
from astramind_mini.data.application import CorporateActionProjectionService
...
def run(args: argparse.Namespace) -> int:
    ...
    ledger.migrate()
    service = CorporateActionProjectionService(
...
def import_lock(path: Path) -> Iterator[None]:
    ...
    with path.open("w", encoding="utf-8") as stream:
        try:
            ...
            raise RuntimeError("公司行为投影已有进程在运行") from error
        yield
