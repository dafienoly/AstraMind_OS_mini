"""Publish official ETF model evidence without any broker or model activation."""
...
async def run(args: argparse.Namespace) -> int:
    ...
    publication = await EtfOfficialEvidenceService(
    ...
    print("historical_mapping_availability_known=false")
    print("etf_v2_gate_ready=false")
...
def publication_lock(path: Path) -> Iterator[None]:
    ...
    with path.open("w", encoding="utf-8") as stream:
        try:
            ...
            raise RuntimeError("ETF 官方证据发布已有进程在运行") from error
        yield
...
def main() -> int:
    ...
    with publication_lock(Path("var/control/etf-model-evidence.lock")):
